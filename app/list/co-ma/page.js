  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setValues(data?.constructionMachineryList || []);
      } catch (error) {
          ...
          <div className="widget-banner-car-listing banner-car-listing-list forklift-bg">
            <div className="themesflat-container full">
              <div className="banner-car-listing">
                <h1 className="title text-white">Construction Machinery</h1>
              </div>
            </div>
          </div>

          <div className="widget-car-listing-list">
            <Categories currentIndex={6} />
                        ...
                        {values &&
                          values.map((val) => {
                            return (
                              <AdCard
                                key={val.id}
                                path={`Construction Machineary|${val.id}`}
                                data={val}
                              />
                            );
                          })}
